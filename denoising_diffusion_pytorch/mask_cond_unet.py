...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb = None):
        print("    Inside ResnetBlock forward:")
...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Calculate output channels for final_conv_layer
        default_out_dim = channels * (1 if not learned_variance else 2)
        self.out_dim = default(out_dim, default_out_dim)
        final_conv_out_channels = self.out_dim * out_mul
        
        block_klass = partial(ResnetBlock, groups=resnet_block_groups)

        self.final_conv_layer = nn.Conv2d(dim, final_conv_out_channels, 1)

        # Layers
        self.final_res_block = block_klass(dim * 2, dim, time_emb_dim=time_dim)
        # self.final_conv = nn.Conv2d(dim, self.out_dim * out_mul, 1) #Removed
        ...
        feature_size_list = [[int(input_size[0] / 2 ** k), int(input_size[1] / 2 ** k)] for k in
                             range(len(dim_mults))]
        ...
        # In the Unet class initialization
        self.decouple1 = nn.Sequential(
            ...
            nn.Conv2d(mid_dim, mid_dim, 1)  # Replace BlockFFT with simple 1x1 conv
        )
    ...
    def forward(self, x, time, mask, x_self_cond=None, **kwargs):
        ...
        print("Shape after init_conv_mask(mask):", [h.shape for h in hm])
        # if self.cond_pe:
        #     m = self.cond_pos_embedding(m)
        x = self.init_conv(torch.cat([x, F.interpolate(hm[0], size=x.shape[-2:], mode="bilinear")], dim=1))
        print("Shape after init_conv(concat[x, hm[0]]):", x.shape)
        r = x.clone()

        t = self.time_mlp(torch.log(time)/4)
        print("Time embedding (t) shape:", t.shape)
        ...
        for i, layer in enumerate(self.projects):
            # print(hm[i].shape)
            hm[i] = layer(hm[i])
            print(f"Shape after projects layer {i}:", hm[i].shape)
        hm2 = []
       ...
       #     print(f"  Shape after block1:", x.shape)
         ...
         #   print(f"  Applying relation_layer with hm[{i}] shape:", hm[i].shape)
            x = relation_layer(hm[i], x)
           # print(f"  Shape after relation_layer:", x.shape)
       ...
       # print("  Shape after mid_block1:", x.shape)
        x = self.mid_attn(x)
      #  print("  Shape after mid_attn:", x.shape)
        x = self.mid_block2(x, t)
       # print("  Shape after mid_block2:", x.shape)

        # Print statements before decouple
     #   print("  Shapes before decouple:")
     #   print("    x:", x.shape)

      #  print("  Applying decouple1")
        x1 = x + self.decouple1(x)
     #   print("  Shape after decouple1 (x1):", x1.shape)
    #    print("  Applying decouple2")
        x2 = x + self.decouple2(x)
    #    print("  Shape after decouple2 (x2):", x2.shape)

        x = x1
        print("  Value of x (x1):", x.shape)
        for i, ((block1, block2, attn, upsample), relation_layer) in enumerate(zip(self.ups, self.relation_layers_up)):
            print(f"\n-- Upsampling Block {i} --")
            print(f"  Input shape (x) to upsampling block:", x.shape)  # Print shape at the beginning of the block
            h_pop = h.pop()
            print(f"h_pop shape: {h_pop.shape}")
            x = torch.cat((x, h_pop), dim=1)
            print(f"  Shape after concat with h:", x.shape)
            x = block1(x, t)
            print(f"  Shape after block1:", x.shape)
            hm_pop = hm.pop()
            print(f"hm_pop shape: {hm_pop.shape}")
            x = relation_layer(hm_pop, x)
            print(f"  Shape after relation_layer:", x.shape)
            h_pop = h.pop()
            print(f"h_pop shape: {h_pop.shape}")
            x = torch.cat((x, h_pop), dim=1)
        ...
        print("Shape after concat with r (x1):", x1.shape)
        x1 = self.final_res_block(x1, t)  # This outputs [1, 128, 80, 80]
        # No need to concatenate again
        x1 = self.final_conv_layer(x1)  # Now expects 128 channels
        print("Shape after final_conv_layer (x1):", x1.shape)

        x = x2
        for i, ((block1, block2, attn, upsample), relation_layer) in enumerate(zip(self.ups2, self.relation_layers_up2)):
            print(f"\n-- Upsampling Block 2.{i} --")
            h2_pop = h2.pop()
            print(f"h2_pop shape: {h2_pop.shape}")
            x = torch.cat((x, h2_pop), dim = 1)
            print(f"  Shape after concat with h2:", x.shape)
            x = block1(x, t)
            print(f"  Shape after block1:", x.shape)
            hm2_pop = hm2.pop()
            print(f"hm2_pop shape: {hm2_pop.shape}")
            x = relation_layer(hm2_pop, x)
            print(f"  Shape after relation_layer:", x.shape)
            h2_pop = h2.pop()
            print(f"h2_pop shape: {h2_pop.shape}")
            x = torch.cat((x, h2_pop), dim = 1)
        ...
        print("Shape after concat with r (x2):", x2.shape)
        x2 = self.final_res_block2(x2, t)
        print("Shape after final_res_block2 (x2):", x2.shape)
        x2 = self.final_conv2(x2)
        print("Shape after final_conv2 (x2):", x2.shape)
        ...
        print("==== End of Unet Forward Pass ====")
        return x1, x2
